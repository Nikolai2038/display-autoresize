...
3. Скачайте скрипт `display-autoresize` из этого репозитория и сделайте его исполняемым:
   ...
   - Вы можете разместить скрипт не в `/usr/local/bin/display-autoresize`, но не забудьте также поменять путь к нему в правиле udev (смотреть ниже).
...
#### 2.1.1. [НЕСТАБИЛЬНО] Менеджер входа (SDDM)
...
- Я попробовал значения `x11-user` и `wayland` для настройки `DisplayServer` какое-то время, и понял, что они гораздо более нестабильны, чем значение по умолчанию `x11`. Конкретнее, настройка `x11-user` приводит к вылету сессии пользователя (после его авторизации) спустя какое-то время. А настройка `wayland` - приводит к зависанию подключения при использовании приложения `aSPICE` на Android. Я досконально не тестировал все эти случаи, но пока что буду использовать стандартное значение `x11`.
